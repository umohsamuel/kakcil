...
import {
  ...
  useAvailableModels,
  useClearCouncilMembers,
} from "@/hooks/use-council";
import { MODEL_DESCRIPTIONS, AIProvider } from "@/types/council";
import { Users, Loader2, Bot, Sparkles, Trash2 } from "lucide-react";
...
export function CouncilMembersSettings() {
  ...
  const updateSelectedModels = useEffectEvent((models: Set<string>) => {
    setSelectedModels(models);
  });

  const updateHasChanges = useEffectEvent((hasChanges: boolean) => {
    ...
    return availableModels.reduce(
      (acc, model) => {
        ...
        acc[model.provider].push({
          ...
          description:
            model.description || MODEL_DESCRIPTIONS[model.model_name],
        });
        return acc;
      },
  ...
  useEffect(() => {
    if (members.length > 0) {
      ...
      updateSelectedModels(activeModels);
    }
  }, [members]);
    ...
    return (
      <Card className="lg:border-border w-full border-2 border-none shadow-none lg:shadow">
        <CardHeader className="px-0 lg:px-6">
          ...
          <CardDescription>
            Loading your council configuration...
          </CardDescription>
        </CardHeader>
        <CardContent className="flex items-center justify-center px-0 py-8 lg:px-6">
  ...
  return (
    <Card className="lg:border-border w-full max-w-full space-y-6 border-none shadow-none lg:space-y-0 lg:border-2 lg:shadow">
      ...
      <CardContent className="space-y-6 p-0 lg:p-6">
        ...
        <div className="border-border bg-muted/50 rounded-lg border p-2 lg:p-4">
        ...
        {(
          Object.entries(modelsByProvider) as [
            AIProvider,
            { model_name: string; description?: string }[],
          ][]
        ).map(([provider, models]) => {
                ...
                {models.map((model) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={cn(
                          "flex h-4 w-4 shrink-0 items-center justify-center rounded-full border-2 transition-colors lg:h-6 lg:w-6 lg:border-[3px]",
                          isSelected
                      ...
                      >
                        {isSelected && (
                          <div className="bg-foreground aspect-square h-2 w-2 rounded-full lg:h-4 lg:w-4" />
                        )}
                      </div>
                      {/* Model Info */}
        ...
        {/* Action Buttons */}
        <div className="border-border flex flex-col items-center justify-between gap-4 border-t pt-4">
